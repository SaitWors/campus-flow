...
for file in COMPLETE auth.dump schedule.dump queue.dump; do
    test -f "$backup_dir/$file" || { echo "Missing backup file: $file" >&2; exit 1; }
done
echo 'This replaces ALL current accounts, schedule and queues with the selected backup.'
...
echo 'Application stays stopped if restoration fails. Do not start it until all three databases are restored.'
