...
export default function LessonForm({rule,lesson,onClose,onSave}:{rule?:Rule;lesson?:Lesson;onClose:()=>void;onSave:()=>void}){
 ...
 function set(key:string,value:string|number|boolean){if(key==='start'||key==='end')timeTouched.current=true;setChangePreview(null);setData(d=>({...d,[key]:value,...(key==='kind'&&value==='lecture'?{queue_enabled:false}:{})}));}
