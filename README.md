...
> **PR3:** справочник предметов, настраиваемое время пар, предпросмотр изменения шаблона, TOTP и сеансы, публичная подписка календаря и офлайн-копия. [Как пользоваться и обновить установку](docs/PR3_RU.md).

> **Сервер 2 vCPU / 2 ГБ / 40 ГБ:** отдельный [production-профиль и инструкция](docs/DEPLOY_TIMEWEB40_RU.md): готовые SHA-образы из GHCR, один PostgreSQL с четырьмя базами, ограниченные ресурсы, перенос старых данных, резервные копии и проверяемый откат. [Результаты проверок и ограничения](docs/PRODUCTION_VALIDATION_RU.md). На реальной Timeweb VM профиль пока не проверен.
...
Сайт учебной группы МТУСИ, 4 курс, направление 09.03.01. Расписание, календарь, очередь на сдачу работ, регистрация по приглашению, роли и администрирование. Интерфейс на русском и английском; светлая, тёмная, чёрная, ультрачёрная и системная темы.
...
- Четыре независимых API: учётные записи, расписание, очереди и уведомления; у каждого отдельная база и роль. В production четыре базы работают внутри одного PostgreSQL; в обычном Compose — четыре экземпляра PostgreSQL.
...
- Локальный автоматический перевод названий RU → EN с ручным приоритетом.
- Пять тем, RU/EN, мобильная компоновка, SVG-иконки Lucide, экспорт iCalendar.
...
| [docs/DEPLOY_TIMEWEB40_RU.md](docs/DEPLOY_TIMEWEB40_RU.md) | Готовые образы, перенос 4 → 1 PostgreSQL, обновление, backup/restore и rollback |
